...
NaturalQL is a small natural-language-to-SQL application built around a clear
trust boundary: the language model proposes a query, but deterministic code
decides whether that query may run.

The demo uses OpenAI for SQL generation, sqlglot for schema-aware validation,
DuckDB for a seeded cinema dataset, and Streamlit for the interface.
